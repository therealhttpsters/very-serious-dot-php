 ...
 * @return array
 */
function veryserious_body_classes( $classes ) {
...
add_filter( 'body_class', 'veryserious_body_classes' );

/**
 * Add a pingback url auto-discovery header for singularly identifiable articles.
 */
function veryserious_pingback_header() {
	if ( is_singular() && pings_open() ) {
		echo '<link rel="pingback" href="', esc_url( get_bloginfo( 'pingback_url' ) ), '">';
	}
}
add_action( 'wp_head', 'veryserious_pingback_header' );
 ...
 * @return array
 */
function veryserious_filter_script_loader_tag( $tag, $handle ) {
...
add_filter( 'script_loader_tag', 'veryserious_filter_script_loader_tag', 10, 2 );
 ...
 */
function veryserious_get_preload_stylesheet_uri( $wp_styles, $handle ) {
 ...
 */
function veryserious_add_body_style() {
	...
	// Preload content.css.
	$preloads['veryserious-content'] = veryserious_get_preload_stylesheet_uri( $wp_styles, 'veryserious-content' );

	// Preload sidebar.css and widget.css.
	if ( is_active_sidebar( 'sidebar-1' ) ) {
		$preloads['veryserious-sidebar'] = veryserious_get_preload_stylesheet_uri( $wp_styles, 'veryserious-sidebar' );
		$preloads['veryserious-widgets'] = veryserious_get_preload_stylesheet_uri( $wp_styles, 'veryserious-widgets' );
	}

	// Preload comments.css.
	if ( ! post_password_required() && is_singular() && ( comments_open() || get_comments_number() ) ) {
		$preloads['veryserious-comments'] = veryserious_get_preload_stylesheet_uri( $wp_styles, 'veryserious-comments' );
	}

	// Preload front-page.css.
	global $template;
	if ( 'front-page.php' === basename( $template ) ) {
		$preloads['veryserious-front-page'] = veryserious_get_preload_stylesheet_uri( $wp_styles, 'veryserious-front-page' );
	}
...
add_action( 'wp_head', 'veryserious_add_body_style' );
 ...
 */
function veryserious_add_primary_menu_dropdown_symbol( $item_output, $item, $depth, $args ) {
...
add_filter( 'walker_nav_menu_start_el', 'veryserious_add_primary_menu_dropdown_symbol', 10, 4 );
 ...
 */
function veryserious_add_nav_menu_aria_current( $atts, $item ) {
	/*
...
add_filter( 'nav_menu_link_attributes', 'veryserious_add_nav_menu_aria_current', 10, 2 );
add_filter( 'page_menu_link_attributes', 'veryserious_add_nav_menu_aria_current', 10, 2 );
